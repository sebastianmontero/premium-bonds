...
function main() {
  ...
  const rawCharters = args.charters ? String(args.charters).split(',') : ['General Exploratory Test'];
  const scope = args.scope || 'Verify recent changes and check core user flows.';
  ...
  const chartersMarkdown = rawCharters.map((charter, index) => {
