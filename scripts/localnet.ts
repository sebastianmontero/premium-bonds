...
export function parseLocalnetFlags(args: string[]): LocalnetFlags {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === "--db" || arg === "-d") {
      ...
      bootstrapOnly = true;
    } else if (arg === "--no-relayer") {
      noRelayer = true;
    } else if (arg === "--from-genesis" || arg === "--backfill") {
      fromGenesis = true;
...
function printUsage() {
  ...
  console.log("  deficit [amount_usdc] [--pool-id <id> | -i <id>]");
  console.log(
...
function writeEnvLocal(
  ...
  console.log("Successfully synchronized .env.local configuration.");
}
...
function cleanupAndExit(code: number = 0) {
  ...
  if (relayerProcess) {
    console.log("Stopping Solana Webhook Relayer...");
    try {
...
`);
  ...
  // 5. Start Solana Webhook Relayer
  if (!flags.noRelayer && !flags.bootstrapOnly) {
    const relayerArgs = ["scripts/webhook-relayer.ts"];
    if (flags.dbName) {
      relayerArgs.push("--db", flags.dbName);
    }
    if (flags.fromGenesis) {
      relayerArgs.push("--from-genesis");
    }
    console.log("Spawning Solana Webhook Relayer...");
    relayerProcess = spawn("npx", ["tsx", ...relayerArgs], {
      stdio: "inherit",
      shell: true,
    });

    relayerProcess.on("error", (err) => {
      console.warn("[Localnet Warning] Failed to spawn Webhook Relayer:", err);
    });
  }
}
...
async function handleDbDelete(args: string[]) {
  ...
  try {
    ...
    const cursorPath = path.resolve(DB_DIR, `${cleanName}.cursor.json`);
...
async function updateMockHumaTotalAssets(
  rpc: ReturnType<typeof createSolanaRpc>,
  ...
  rawData.writeBigUInt64LE(newTotalAssets >> 64n, 38);
...
async function handleYield(args: string[]) {
  ...
  await updateMockHumaTotalAssets(
    rpc,
    addresses.humaPoolState,
    requiredTotalAssets
  );
...
async function handleDeficit(args: string[]) {
  let amountUsdcStr: string | undefined = undefined;
  let poolId = 1;
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === "--pool-id" || arg === "-i") {
      ...
      console.error(
        `Error: Unknown argument '${arg}'. Usage: npm run localnet deficit [amount_usdc] [--pool-id <id> | -i <id>]`
      );
      process.exit(1);
    }
  }

  // Default to DEFAULT_DEFICIT_USDC (1.0 USDC) if amount is omitted
  const deficitAmountFloat =
    amountUsdcStr !== undefined
      ? parseFloat(amountUsdcStr)
      : DEFAULT_DEFICIT_USDC;

  if (isNaN(deficitAmountFloat) || deficitAmountFloat <= 0) {
    console.error(
      `Error: Invalid deficit amount '${amountUsdcStr}'. Must be a positive number.`
    );
    process.exit(1);
  }

  // USDC has 6 decimals (1 USDC = 1_000_000 micro-USDC)
  const deficitMicroUsdc = BigInt(Math.round(deficitAmountFloat * 1_000_000));

  if (deficitMicroUsdc <= SOLVENCY_DUST_TOLERANCE_BASE_UNITS) {
    console.warn(
      `\n⚠️  Advisory Warning: Specified deficit (${deficitAmountFloat} USDC / ${deficitMicroUsdc} micro-units) is <= SOLVENCY_DUST_TOLERANCE (${SOLVENCY_DUST_TOLERANCE_BASE_UNITS} base units / 0.001 USDC).\n` +
  ...
  if (poolPstBalance === 0n) {
    console.error(
      "Error: Pool PST balance is 0. Cannot simulate deficit without active deposits."
    );
    process.exit(1);
  }

  let requiredTotalAssets: bigint;
  try {
    requiredTotalAssets = calculateDeficitTotalAssets({
      bookValue,
      deficitMicroUsdc,
      pstSupply,
      poolPstBalance,
  ...
  console.log(`- Pool Book Value: ${Number(bookValue) / 1_000_000} USDC`);
  console.log(
    `- Target Deficit: ${deficitAmountFloat} USDC (${deficitMicroUsdc} micro-USDC)`
  );
  console.log(
    `- Target Current Value: ${Number(targetCurrentValue) / 1_000_000} USDC`
  );
  console.log(`- PST Supply: ${pstSupply}`);
  console.log(`- Pool PST Balance: ${poolPstBalance}`);
  console.log(
  ...
  console.log(
    `Successfully induced deficit! Huma pool total assets updated to ${Number(requiredTotalAssets) / 1_000_000} USDC.`
  );
  console.log(
    `👉 Next step: Run 'npm run localnet draw --pool-id ${poolId}' or 'npx tsx scripts/pb-cli.ts harvest -i ${poolId}' to trigger the Solvency Circuit Breaker and transition the draw cycle to 'HaltedInsolvent'.`
  );
}
